...
- **Markdown:** `squads/goals-tracker/output/2026-03-26-112700/v1/leadership-report.md` — seções nesta ordem: `## Resumo executivo` → `## Status das metas (RAG)` → **`## Andamento por frente (explícito)`** → **`## Impedimentos e decisões (Five Whats e 5W2H)`** (tabela **# | O quê | Por quê | Onde | Quando | Quem | Como | Quanto**) → `## Próximos passos recomendados (liderança)`.
- **HTML:** `.../2026-03-26-112700/v1/leadership-report.html` — `progressChart` + `frentes-table` + **`w2h-wrap` / `w2h-table`** nos impedimentos; **sem** painel gestor, doughnuts extras, A–E nem segundo gráfico de governança.

Instruções normativas: `pipeline/steps/step-03-write.md` (veto se desviar). A camada de **frentes** é obrigatória; não reintroduzir o layout longo antigo (painel gestor por badges, seções A–E) sem alteração explícita deste arquivo e do passo 03.
